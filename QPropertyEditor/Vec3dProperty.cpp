...
QVariant Vec3dProperty::value(int role) const
{
	...
	if (data.isValid() && role != Qt::UserRole)
	{
		...
		{
		case Qt::DisplayRole:
			return tr("[ %1, %2, %3]").arg(data.value<Vec3d>().X,0,'g',10).arg(data.value<Vec3d>().Y,0,'g',10).arg(data.value<Vec3d>().Z,0,'g',10);
		case Qt::EditRole:
			return tr("%1, %2, %3").arg(data.value<Vec3d>().X,0,'g',10).arg(data.value<Vec3d>().Y,0,'g',10).arg(data.value<Vec3d>().Z,0,'g',10);
		};
	}
...
void Vec3dProperty::setValue(const QVariant& value)
{
	if (value.type() == QVariant::String)
	{
		...
		m_y->setProperty("y", y);
		m_z->setProperty("z", z);
		Property::setValue(QVariant::fromValue(Vec3d(x, y, z)));
	}
	else
		Property::setValue(value);
}

void Vec3dProperty::setEditorHints(const QString& hints)
{
...
double Vec3dProperty::x() const
{
	return value().value<Vec3d>().X;
}

void Vec3dProperty::setX(double x)
{
	Property::setValue(QVariant::fromValue(Vec3d(x, y(), z())));
}

double Vec3dProperty::y() const
{
	return value().value<Vec3d>().Y;
}

void Vec3dProperty::setY(double y)
{
	Property::setValue(QVariant::fromValue(Vec3d(x(), y, z())));
}

double Vec3dProperty::z() const
{
	return value().value<Vec3d>().Z;
}

void Vec3dProperty::setZ(double z)
{
	Property::setValue(QVariant::fromValue(Vec3d(x(), y(), z)));
}

QString Vec3dProperty::parseHints(const QString& hints, const QChar component )
{
