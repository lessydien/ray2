...
void QPropertyModel::addItem(QObject *propertyObject)
{
	...
	// search for name property of curren PropertyObject
	QVariant varName=propertyObject->property("Name");
	// set default name
...
//			name = metaObject->classInfo(index).value();
		...
		foreach(PropertyPair pair, propertyMap)
		{
			...
			if (pair.Object == metaObject)
			{
				...
				QString testString=property.name();
				QVariant t_type=property.type();
				Property* p = 0;
