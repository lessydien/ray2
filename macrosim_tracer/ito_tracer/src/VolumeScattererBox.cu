...
rtDeclareVariable(VolumeScattererBox_ReducedParams, params, , ); // normal vector to surface. i.e. part of the definition of the plane surface geometry
...
{
	  double t=intersectRayVolumeScattererBox(prd.position, prd.direction, params);
	  ...
	  {
		hitParams=calcHitParamsVolumeScattererBox(prd.position+t*prd.direction, params);
...
RT_PROGRAM void bounds (int, float result[6])
{
  ...
  // sag of lens
  float3 maxBox=make_float3(params.root+params.normal*params.thickness+params.apertureRadius.x*l_ex+params.apertureRadius.y*l_ey);
