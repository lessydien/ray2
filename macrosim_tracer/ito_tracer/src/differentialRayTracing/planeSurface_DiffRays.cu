...
rtDeclareVariable(PlaneSurface_DiffRays_ReducedParams, params, , ); // normal vector to surface. i.e. part of the definition of the plane surface geometry
...
__forceinline__ __device__ Mat_DiffRays_hitParams calcHitParams(double t)
{
  ...
  t_hitParams.mainDirX=rotateRay(make_double3(1,0,0),params.tilt);
  t_hitParams.mainDirY=rotateRay(make_double3(0,1,0),params.tilt);
  // calc main radii of curvature at intersection. DOUBLE_MAX is the closest we can get to infinity here...
  t_hitParams.mainRad=make_double2(DOUBLE_MAX,DOUBLE_MAX);
...
{
	  double t=intersectRayPlaneSurface_DiffRays(prd.position, prd.direction, params);
