...
__device__ void reflecting_torrSparr1D_anyHit_device()
{
...
__device__ void reflecting_torrSparr1D_closestHit_device( Mat_DiffRays_hitParams hitParams, double t_hit )
{
  if( prd.depth <= max_depth ) 
  {
    ...
    if (hitCoatingNumCoeff_DiffRays(prd, hitParams, coating_params)) // see weter coating wants this material to be reflecting
		hitReflecting_DiffRays(prd, hitParams, t_hit, geometryID); // if coating wants reflections, reflect. otherwise don't alter the rays direction
...
{
  reflecting_torrSparr1D_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  reflecting_torrSparr1D_closestHit_device( hitParams, t_hit ); 
}
