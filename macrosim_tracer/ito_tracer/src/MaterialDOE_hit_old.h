...
//inline RT_HOSTDEVICE bool hitDOE(rayStruct &ray, Mat_hitParams hitParams, MatDOE_params params, double t_hit, int geomID, bool coat_reflected)
{
	...
	// --->>>  rotateRayInv(&n, tiltrad);
	rotateRay(&n, tiltrad);

	double3 n2 = hitParams.normal;
	double3 n3 = hitParams.normal;
	double3 n4 = hitParams.normal;

	rotateRayInv(&n2, -tiltrad);
	rotateRay(&n3, tiltrad);
	rotateRay(&n4, -tiltrad);
	...
	double3 Local;
	double3 zeroposition2 = ray.position - root;
	rotateRayInv(&zeroposition2, tiltrad);

	double3 zerodirection = params.geomTilt*2*PI/360;
	rotateRay(&zerodirection, tiltrad);
	zerodirection.x = 0;
	zerodirection.y = 0;
	double3 zeroposition = ray.position - root;
	rotateRay(&zeroposition, tiltrad);

	rotateRay(&zeroposition, zerodirection);
...
if (firstOrder == true)
{
	...
	//rotateRay(&ray.direction, params.geomTilt);
...
//	double nn = 1;
	...
	// --->>>  rotateRayInv(&ray.direction, tiltrad);
	rotateRay(&ray.direction, tiltrad);
	//rotateRay(&ray.direction, zerodirection);
