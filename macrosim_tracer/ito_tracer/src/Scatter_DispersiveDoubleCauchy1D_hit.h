...
#ifndef SCATTER_DISPDOUBLECAUCHY1D_HIT_H
  #define SCATTER_DISPDOUBLECAUCHY1D_HIT_H
...
/* declare class */
/**
  *\class   ScatDispersiveDoubleCauchy1D_params
  *\ingroup Scatter
  ...
  *
  */
class ScatDispersiveDoubleCauchy1D_params: public ScatDoubleCauchy1D_params
{
public:
...
/**
 * \detail hitDispersiveDoubleCauchy1D 
 *
 * modifies the raydata according to the parameters of the scatter
 *
 * \param[in] rayStruct &prd, Mat_hitParams hitParams, ScatDispersiveDoubleCauchy1D_params params
 * 
 * \return bool
 * \sa 
 ...
 * \author Mauch
 */
inline RT_HOSTDEVICE bool hitDispersiveDoubleCauchy1D(rayStruct &prd, Mat_hitParams hitParams, ScatDispersiveDoubleCauchy1D_params params)
{
	ScatDispersiveDoubleCauchy1D_params test=params;
	uint32_t x1[5];
	...
	double specAngle=acos(dot(normalize(eIn1),-hitParams.normal)); // calc angle of ray to normal
	double3 rotAxis=cross(params.scatAxis,hitParams.normal); // calc axis around which these vectors are rotated to each other
	
	// calc scatter angle
	double xi=(Random(x1)-0.5)*PI; // get uniform distribution between -pi/2 and pi/2
	double BRDFmax=params.Ksl+params.Ksp;
	double ii=Random(x1)*BRDFmax; // get uniform distribution between zero and maximum BRDF to decide the parameter set of which cauchy we need here
	double gamma=params.gammaXsp; // init sigma to sigma of specular peak
	if (ii>params.Ksp)
	{
		prd.depth++;
		gamma=params.gammaXsl; // set sigma to sigma of specular lobe
	}
	xi=gamma*tan(xi); // transform into cauchy distributed variable
...
//inline RT_HOSTDEVICE bool hitDispersiveDoubleCauchy1D(rayStruct &prd, double3 hitParams.normal, ScatDispersiveDoubleCauchy1D_params params)
//{
//	uint32_t x1[5];
