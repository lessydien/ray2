...
inline RT_HOSTDEVICE bool hitLambert2D(rayStruct &prd, Mat_hitParams hitParams, ScatLambert2D_params params)
{
	...
	double intensityCosLambertian=dot(hitParams.normal,prd.direction)/sqrt(abs(dot(hitParams.normal,hitParams.normal)*dot(prd.direction,prd.direction)));
	...
	//prd.flux=prd.flux*sqrt(params.TIR)*intensityCosLambertian;
	//prd.flux=prd.flux;
	...
	if (params.impAreaType==AT_INFTY)
	{
		...
		//prd.direction=hitParams.normal/sqrt(dot(hitParams.normal,hitParams.normal));
		...
		prd.flux=prd.flux*params.TIR*intensityCosLambertian;
