...
{
//	if (!scalar_RichardsonWolf_wrapper(Uin_ptr, dimx, dimy, wvl, x1_ptr, y1_ptr, f, Dz, x2_ptrptr, y2_ptrptr))
//		return PROP_ERR;
	return PROP_NO_ERR;
}
...
propError cu_ft2(complex<double>* Uin_ptr, unsigned int dimx, unsigned int dimy, double delta)
{
    ...
    if (!myCudaSafeCall(cudaMalloc((void**)&Uin_kernel_ptr, sizeof(complex<double>)*dimx*dimy),__FILE__, __LINE__))
		return PROP_ERR;
	//(cudaMalloc((void**)&Uin_kernel_ptr, sizeof(complex<double>)*dimx*dimy));

    // Copy host memory to device
    if (!myCudaSafeCall(cudaMemcpy(Uin_kernel_ptr, Uin_ptr, sizeof(complex<double>)*dimx*dimy, cudaMemcpyHostToDevice),__FILE__, __LINE__))
		return PROP_ERR;
    ...
    // CUFFT plan
    cufftHandle plan;
    if (!myCufftSafeCall(cufftPlan2d(&plan,dimx, dimy, CUFFT_Z2Z),__FILE__, __LINE__))
		return PROP_ERR;
	//(cufftPlan2d(&plan,dimx, dimy, CUFFT_Z2Z));

    // execution
    if (!myCufftSafeCall(cufftExecZ2Z(plan, (cufftDoubleComplex *)Uin_kernel_ptr, (cufftDoubleComplex *)Uin_kernel_ptr, CUFFT_FORWARD),__FILE__, __LINE__))
		return PROP_ERR;
	...
	if (!myCudaSafeCall(cudaMemcpy(Uin_ptr, Uin_kernel_ptr, sizeof(complex<double>)*dimx*dimy, cudaMemcpyDeviceToHost),__FILE__, __LINE__))
		return PROP_ERR;
	...
	if (!myCudaSafeCall(cudaFree(Uin_kernel_ptr),__FILE__, __LINE__))
		return PROP_ERR;
...
propError cu_scalar_RichardsonWolf(complex<double>* Uin_ptr, unsigned int dimx, unsigned int dimy, double wvl, double* x1_ptr, double* y1_ptr, double f, double Dz, double** x2_ptrptr, double** y2_ptrptr)
{
	...
	cu_ft2(Uin_ptr, dimx, dimy, dx1/f);
