...
/**
  ...
  *
  * - \b rtLaunchIndex - rtLaunchIndex is the launch invocation index. Type must be one of \a unsigned int, \a uint2, \a uint3, \a int, \a int2, \a int3 and is read-only.
  * - \b rtLaunchDim - rtLaunchDim is the size of each dimension of the launch. The values range from 1 to the launch size in that dimension. Type must be one of \a unsigned int, \a uint2, \a uint3, \a int, \a int2, \a int3 and is read-only.
  * - \b rtCurrentRay - rtCurrentRay is the currently active ray, valid only when a call to @ref rtTrace is active.  Type must be \a optix::Ray and is read-only.
  ...
  * - \b rtRayPayload - rtRayPayload refers to the struct passed into the most recent @ref rtTrace call and is read-write.
  * - \b attribute \a name - refers to a named attribute passed from the intersect program to a closest-hit or any-hit program.  The types must match in both sets of programs.  This variable is read-only in the closest-hit or any-hit program and is written in the intersection program.
  *
...
template<> struct rtCallableProgramSizeofWrapper<void> { static const size_t value = 0; };
  ...
  * to be a function pointer with the specified return type and list of arguments.
  ...
  *@endcode
  * 
  * @param[in]  returnType   Return type of the callable program
  * @param[in]  name         Name of the callable program
  * @param[in]  parameters   Parameters of the callable program
  * 
  * <B>History</B>
  * 
...
/**
  ...
  * output when @ref rtContextLaunch finishes. The buffer size can be configured using
