...
extern "C" {
  ...
  * In case where the user provides a single external data pointer for a buffer prior to calling @ref rtContextLaunch,
  ...
  *
  * Once OptiX has copied the data to other devices from the one where the user has specified a device pointer,
  * it will not do so again until @ref rtBufferMarkDirty has been called.
  *
  ...
  * @param[in]   type             Bitwise \a or combination of the \a type and \a flags of the new buffer
