...
geometryGroupError GeometryGroup::createOptixInstance(RTcontext &context, RTgroup &OptiX_group, unsigned int index, simMode mode, double lambda)
{
	...
	if (1)//mode==SIM_GEOMRAYS_NONSEQ)
	{
		...
		for (i=0; i<geometryListLength; i++)
		{
			...
			{
				std::cout << "error in GeometryGroup.createOptixInstance: geometryList[i]-YcreateOptiXInstance returned an error at index:" << i << std::endl;
				return GEOMGROUP_ERR;
			}
		}

		/* connect OptiX_geometrygroup to OptiX_group */
		if (!RT_CHECK_ERROR_NOEXIT( rtGroupSetChild( OptiX_group, index, OptiX_geometrygroup ), context ))
			return GEOMGROUP_ERR;
