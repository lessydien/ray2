...
__device__ void linearGrating1D_closestHit_device( double3 p_normal, double t_hit )
{
  if (prd.depth < max_depth)
  {
    ...
    if (params.nRefr1==0)
		reflected=true;
    hitLinearGrating1D(prd, p_normal, params, t_hit, geometryID, reflected);
  }
  else
  {
...
{
  linearGrating1D_closestHit_device( geometric_normal, t_hit );
}
