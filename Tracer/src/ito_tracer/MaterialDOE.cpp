...
//	extern Group oGroup;
	...
	//if ( hitDOE(ray, hitParams, this->params, t_hit, geometryID, coat_reflected) )
	{
 ...
 * \author Mauch
 */
void MaterialDOE::hit(gaussBeamRayStruct &ray, gaussBeam_geometricNormal normal, int geometryID)
{
	extern Group oGroup;
	double n=1; 
	...
	else
	{
		...
		std::cout << "error in MaterialDOE.hit: ray is immersed in an medium that is not attached to material" << std::endl;
	}
