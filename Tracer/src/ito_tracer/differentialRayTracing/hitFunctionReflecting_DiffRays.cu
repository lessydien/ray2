...
__forceinline__ __device__ void reflectingDiffRays_anyHit_device()
{
...
__forceinline__ __device__ void reflectingDiffRays_closestHit_device( Mat_DiffRays_hitParams hitParams, double t_hit )
{
  // reflection ray
  if (prd.depth < max_depth) 
  {
	...
	rtPrintf("flux %.20lf \n", prd.flux);
    hitReflecting_DiffRays(prd, hitParams, t_hit, geometryID);
  }
  else
  {
...
{
  reflectingDiffRays_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  reflectingDiffRays_closestHit_device( hitParams, t_hit );
}
