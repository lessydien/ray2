...
#ifndef MATERIALLINEARGRATING1D_DIFFRAYS_HIT_H
#define MATERIALLINEARGRATING1D_DIFFRAYS_HIT_H
...
/* declare class */
/**
  *\class   MatLinearGrating1D_DiffRays_params 
  ...
  *
  */
class MatLinearGrating1D_DiffRays_params : public MatLinearGrating1D_params
{
public:
...
/**
 * \detail hitLinearGrating1D_DiffRays 
 *
 * modifies the raydata according to the parameters of the material
 *
 * \param[in] diffRayStruct &ray, Mat_DiffRays_hitParams hitParams, MatLinearGrating1D_params params, double t_hit, int geomID, bool reflected
 * 
 * \return bool
 * \sa 
 ...
 * \author Mauch
 */
inline RT_HOSTDEVICE bool hitLinearGrating1D_DiffRays(diffRayStruct &ray, Mat_DiffRays_hitParams hitParams, MatLinearGrating1D_params params, double t_hit, int geomID, bool reflected)
{
	...
	if (params.nrDiffOrders!=1)
	{
		...
		for (count=0;count<2000;count++)//while (yi>=fxi) // this could turn into an endless loop !! maybe we should implement some break backup
		{
			...
			yi=yi*params.eff[0]; // random between 0 and maximum diffraction efficiency
