...
/**\file PlaneSurface_DiffRays.cpp
...
geometryError PlaneSurface_DiffRays::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, simMode mode, double lambda )
{
	...
	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(PlaneSurface_DiffRays_ReducedParams), this->reducedParamsPtr), context) )
		return GEOM_ERR;
