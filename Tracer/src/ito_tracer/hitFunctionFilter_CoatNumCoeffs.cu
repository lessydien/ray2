...
rtDeclareVariable(MatFilter_params,               params, , );
...
__forceinline__ __device__ void filter_anyHit_device()
{
...
__forceinline__ __device__ void filter_closestHit_device( Mat_hitParams hitParams, double t_hit )
{
  rtPrintf("closest hit ID %i \n", geometryID);
  rtPrintf("flux %.20lf \n", prd.flux);
  hitFilter(prd, hitParams, params, t_hit, geometryID);
  if (hitCoatingNumCoeff(prd, hitParams, coating_params))
  {
	prd.running=true; //keep ray alive;
...
{
  filter_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  filter_closestHit_device( hitParams, t_hit );
}
