 ...
 * \author Mauch
 */
void MaterialDiffracting::hit(gaussBeamRayStruct &ray, gaussBeam_geometricNormal normal, int geometryID)
{
	extern Group oGroup;
	double n=1; 
	...
	else
	{
		...
		std::cout << "error in MaterialDiffracting.hit: ray is immersed in an medium that is not attached to material" << std::endl;
	}
 ...
 * \author Mauch
 */
MaterialError MaterialDiffracting::createOptiXInstance(RTcontext context, RTgeometryinstance &instance, int index, simMode mode, double lambda)
{
 ...
 * \author Mauch
 */
MaterialError MaterialDiffracting::updateOptiXInstance(RTcontext context, RTgeometryinstance &instance, int index, simMode mode, double lambda)
{
