...
detError DetectorIntensity::detect2TextFile(FILE* hFile, RayField* rayFieldPtr)
{
	...
	{
		std::cout << "error in DetectorIntensity.detect2TextFile(): detect() returned an error" << std::endl;
		return DET_ERROR;
	}
...
detError DetectorIntensity::detect(Field* rayFieldPtr, Field **imagePtrPtr)
{
	...
	if (*imagePtrPtr != NULL)
	{
		...
		{
			std::cout << "error in DetectorIntensity.detect: given IntensityField does not match parameters of detector" << std::endl;
			return DET_ERROR;
		}
	}
	else
	{
		...
		{
			std::cout << "error in DetectorIntensity.detect: pixel number smaller than one in y is not allowed" << std::endl;
			return DET_ERROR;
		}
		else
		...
		{
			std::cout << "error in DetectorIntensity.detect: pixel number smaller than one in z is not allowed" << std::endl;
			return DET_ERROR;
		}
		else
		{
