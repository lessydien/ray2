...
#include "Scatter_CookTorrance_GeomRender_hit.h"
...
rtDeclareVariable(ScatCookTorrance_params, scatterParams, , );

/****************************************************************************/
/*				device functions											*/
/****************************************************************************/

__forceinline__ __device__ void reflectingScatterCookTorranceGeomRender_anyHit_device()
{
...
__forceinline__ __device__ void reflectingScatterCookTorranceGeomRender_closestHit_device( Mat_GeomRender_hitParams hitParams, double t_hit )
{
  ...
  if (hitCookTorrance_GeomRender(sdRay, hitParams, scatterParams)) // do the scattering
  {
...
//  prd.running=false;
  ...
  ScatCookTorrance_params primParams=scatterParams;
  primParams.impAreaType=AT_INFTY; // primary ray does not use the importance area

  if (!hitCookTorrance_GeomRender(prd, hitParams, primParams))
      prd.running=false;
...
{
  reflectingScatterCookTorranceGeomRender_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  reflectingScatterCookTorranceGeomRender_closestHit_device( hitParams, t_hit );
}
