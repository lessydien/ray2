...
/**\file GeometryLib_GeomRender.cpp
* \brief 
* 
*           
* \author Mauch
...
bool GeometryFab_GeomRender::createGeomInstFromXML(xml_node &geomNode, SimParams simParams, vector<Geometry*> &geomVec) const
{	
	...
	{
		std::cout << "error in GeometryFab_GeomRender.createGeomInstanceFromXML(): objectType is not defined for given node." << "...\n";
	...
	{
		std::cout << "error in GeometryFab_GeomRender.createInstanceFromXML(): geomType is not defined for given node." << "...\n";
		return false;
	}
	...
	//	break;
	case GEOM_PLANESURF:
		l_pGeometry=new PlaneSurface_GeomRender(1);
		break;
	...
	default:
		cout << "error om GeometryFab_GeomRender.createInstanceFromXML(): unknown geometryType for geometric ray tracing." << endl;
		return false;
		break;
	}
