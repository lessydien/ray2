...
rtDeclareVariable(ScatPhong_params, scatterParams, , );

/****************************************************************************/
/*				device functions											*/
/****************************************************************************/

__forceinline__ __device__ void reflectingScatterPhongGeomRender_anyHit_device()
{
...
__forceinline__ __device__ void reflectingScatterPhongGeomRender_closestHit_device( Mat_GeomRender_hitParams hitParams, double t_hit )
{
  ...
  if (hitPhong_GeomRender(sdRay, hitParams, scatterParams)) // do the scattering
  {
...
//  prd.running=false;
  ...
  ScatPhong_params primParams=scatterParams;
  primParams.impAreaType=AT_INFTY; // primary ray does not use the importance area

  if (!hitPhong_GeomRender(prd, hitParams, primParams))
      prd.running=false;
...
{
  reflectingScatterPhongGeomRender_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  reflectingScatterPhongGeomRender_closestHit_device( hitParams, t_hit );
}
