...
/**\file IdealLense_GeomRender.cpp
* \brief ideal lense
* 
*           
* \author Mauch
 ...
 * \author Mauch
 */
geometryError IdealLense_GeomRender::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
	if (GEOM_NO_ERR != Geometry::createOptixInstance(context, geometrygroup, index, simParams, lambda) )
	{
		std::cout <<"error in IdealLense_GeomRender.createOptixInstance(): Geometry.creatOptiXInstacne() returned an error at geometry: " << this->paramsPtr->geometryID << "...\n";
		return GEOM_ERR;
	}

	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(IdealLense_GeomRender_ReducedParams), this->reducedParamsPtr), context) )
		return GEOM_ERR;
 ...
 * \author Mauch
 */
geometryError IdealLense_GeomRender::updateOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
	if (this->update)
	{
		if (GEOM_NO_ERR != Geometry::updateOptixInstance(context, geometrygroup, index, simParams, lambda))
		{
			std::cout <<"error in IdealLense_GeomRender.updateOptixInstance(): materialList[i] returned an error at geometry: " << this->getParamsPtr()->geometryID << "...\n";
			return GEOM_ERR;
		}
