...
/**\file Scatter_Phong_GeomRender.cpp
...
#include "Scatter_Phong_GeomRender.h"
...
void Scatter_Phong_GeomRender::hit(rayStruct &ray, Mat_hitParams hitParams)
{
	extern Group *oGroup;
    ...
    sdRay.secondary_nr++;
    sdRay.cumFlux=0;
    if (hitPhong_GeomRender(sdRay, hitParams, this->reducedParams) )
    {
...
//    ray.running=false;
    ...
    ScatPhong_params primParams=this->reducedParams;
    primParams.impAreaType=AT_INFTY; // primary ray does not use the importance area
	if (hitPhong_GeomRender(*l_pRay, hitParams, primParams) )
	{
...
ScatterError Scatter_Phong_GeomRender::createOptiXInstance(double lambda, char** path_to_ptx_in)
{
...
ScatterError Scatter_Phong_GeomRender::createCPUSimInstance(double lambda)
{
	// calc the refractive indices at current wavelength
    this->reducedParams.coefLambertian=this->fullParamsPtr->coefLambertian;
    this->reducedParams.coefPhong=this->fullParamsPtr->coefPhong;
