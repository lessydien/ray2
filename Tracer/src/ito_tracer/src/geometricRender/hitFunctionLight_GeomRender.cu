...
rtDeclareVariable(MatLight_GeomRender_params, params, , ); 
...
__forceinline__ __device__ void light_geomRender_anyHit_device()
{
...
__forceinline__ __device__ void light_geomRender_closestHit_device( Mat_hitParams hitParams, double t_hit )
{
  if (prd.depth < max_depth)
  {
    ...
    hitRenderLight(prd, hitParams, params, t_hit, geometryID);
  }
  else
  {
    rtPrintf("ray stopped in hitRenderLight!!");
...
{
  light_geomRender_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  light_geomRender_closestHit_device( hitParams, t_hit );
}
