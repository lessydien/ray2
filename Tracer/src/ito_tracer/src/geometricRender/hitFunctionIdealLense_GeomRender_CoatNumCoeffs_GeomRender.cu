...
__forceinline__ __device__ void idealLenseCoatNumCoeffsGeomRender_anyHit_device()
{
...
__forceinline__ __device__ void idealLenseCoatNumCoeffsGeomRender_closestHit_device( Mat_GeomRender_hitParams hitParams, double t_hit )
{
  rtPrintf("closest hit ID %i \n", geometryID);
  rtPrintf("flux %.20lf \n", prd.flux);
  bool coat_reflected=hitCoatingNumCoeff_GeomRender(prd, hitParams, coating_params);
...
{
  idealLenseCoatNumCoeffsGeomRender_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  idealLenseCoatNumCoeffsGeomRender_closestHit_device( hitParams, t_hit );
}
