...
__forceinline__ __device__ void reflectingScatTorrSparr1DCoatNumCoeffs_anyHit_device()
{
...
__forceinline__ __device__ void reflectingScatTorrSparr1DCoatNumCoeffs_closestHit_device( Mat_hitParams hitParams, double t_hit )
{
    bool coat_reflected=hitCoatingNumCoeff(prd, hitParams, coating_params); // see wether coating wants this material to be reflecting
	hitReflecting(prd, hitParams, t_hit, geometryID, coat_reflected); // if coating wants reflections, reflect. otherwise don't alter the rays direction
...
{
  reflectingScatTorrSparr1DCoatNumCoeffs_anyHit_device();
}


RT_PROGRAM void closestHit()
{
  reflectingScatTorrSparr1DCoatNumCoeffs_closestHit_device( hitParams, t_hit ); 
}
