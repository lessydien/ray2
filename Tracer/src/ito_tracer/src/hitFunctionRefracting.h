...
__device__ void refracting_closestHit_device( double3 p_normal, double t_hit )
{
  ...
  if (prd.depth < max_depth)
  {
	...
	prd=new_prd;
    //prd.position = p_normal;
    //prd.direction = p_normal;
    //prd.flux = new_prd.flux;
    //prd.depth = new_prd.depth;
  }
  else
  {
