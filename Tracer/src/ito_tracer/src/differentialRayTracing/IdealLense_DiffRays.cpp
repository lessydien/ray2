 ...
 * \author Mauch
 */
double IdealLense_DiffRays::intersect(diffRayStruct *ray)
{
	return intersectRayIdealLense_DiffRays(ray->position,ray->direction,*(this->reducedParamsPtr));
};
 ...
 * \author Mauch
 */
geometryError IdealLense_DiffRays::hit(diffRayStruct &ray, double t)
{
 ...
 * \author Mauch
 */
geometryError IdealLense_DiffRays::reduceParams(void)
{
 ...
 * \author Mauch
 */
geometryError IdealLense_DiffRays::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
	if (GEOM_NO_ERR != Geometry::createOptixInstance(context, geometrygroup, index, simParams, lambda) )
	{
		std::cout <<"error in IdealLense_DiffRays.createOptixInstance(): Geometry.creatOptiXInstacne() returned an error at geometry: " << this->paramsPtr->geometryID << std::endl;
		return GEOM_ERR;
	}

	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(IdealLense_DiffRays_ReducedParams), this->reducedParamsPtr), context) )
		return GEOM_ERR;
 ...
 * \author Mauch
 */
geometryError IdealLense_DiffRays::updateOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
	if (this->update)
	{
		if (GEOM_NO_ERR != Geometry::updateOptixInstance(context, geometrygroup, index, simParams, lambda))
		{
			std::cout <<"error in IdealLense_DiffRays.updateOptixInstance(): materialList[i] returned an error at geometry: " << this->getParamsPtr()->geometryID << std::endl;
			return GEOM_ERR;
		}
