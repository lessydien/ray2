 ...
 * \author Mauch
 */
double ConePipe_DiffRays::intersect(diffRayStruct *ray)
{
	double t=intersectRayConePipe_DiffRays(ray->position,ray->direction,*(this->reducedParamsPtr));
	if (t>MAX_TOLERANCE)
	{
		return t;
	}
 ...
 * \author Mauch
 */
geometryError ConePipe_DiffRays::hit(diffRayStruct &ray,double t)
{
	Mat_hitParams hitParams;
	hitParams=calcHitParamsConePipe_DiffRays(ray.position+t*ray.direction, *(this->reducedParamsPtr));
 ...
 * \author Mauch
 */
geometryError ConePipe_DiffRays::reduceParams(void)
{
 ...
 * \author Mauch
 */
geometryError ConePipe_DiffRays::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
 ...
 * \author Mauch
 */
geometryError ConePipe_DiffRays::updateOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
