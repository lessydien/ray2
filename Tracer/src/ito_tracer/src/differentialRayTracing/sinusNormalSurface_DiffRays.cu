...
rtDeclareVariable(SinusNormalSurface_DiffRays_ReducedParams, params, , ); // normal vector to surface. i.e. part of the definition of the plane surface geometry
...
{
  return calcHitParamsSinusNormalSurface_DiffRays(prd.position+t*prd.direction, params);
}

/* calc intersection of ray with geometry */
RT_PROGRAM void intersect(int)
{
	  double t=intersectRaySinusNormalSurface(prd.position, prd.direction, params);
...
RT_PROGRAM void bounds (int, float result[6])
{
  ...
  float3 maxBox=make_float3(params.root+params.normal*params.ampl+params.apertureRadius.x*l_ex+params.apertureRadius.y*l_ey);
  float3 minBox=make_float3(params.root-params.normal*params.ampl-params.apertureRadius.x*l_ex-params.apertureRadius.y*l_ey);
