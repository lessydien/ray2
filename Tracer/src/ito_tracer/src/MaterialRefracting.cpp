...
//	extern Group oGroup;
	...
	if ( hitRefracting(ray, hitParams, this->params, t_hit, geometryID, coat_reflected) )
	{
		...
		cout << "error in MaterialRefracting.hit(): a ray is immersed in " << ray.nImmersed << ". It has hit the geometry " << geometryID << " which is not immersed in that medium!! ray will be stopped." << endl;
