 ...
 * \author Mauch
 */
Geometry_Params* ApertureArraySurface::getParamsPtr(void)
{
 ...
 *
 * \param[in] ApertureArraySurface_Params *paramsInPtr
 * 
 ...
 * \author Mauch
 */
geometryError ApertureArraySurface::setParams(Geometry_Params *paramsIn)//PlaneSurface_Params *paramsIn)
{
	ApertureArraySurface_Params *l_ptr=dynamic_cast<ApertureArraySurface_Params*>(paramsIn);
 ...
 * \author Mauch
 */
double ApertureArraySurface::intersect(rayStruct *ray)
{
	double t=intersectRayApertureArraySurface(ray->position,ray->direction,*(this->reducedParamsPtr));
	if (t>MAX_TOLERANCE)
	{
		return t;
	}
 ...
 * \author Mauch
 */
geometryError ApertureArraySurface::hit(rayStruct &ray, double t)
{
	Mat_hitParams hitParams;
	hitParams=calcHitParamsApertureArraySurface(ray.position+t*ray.direction, *(this->reducedParamsPtr));
	int i;
...
// * \author Mauch
// */
//gaussBeam_t ApertureArraySurface::intersect(gaussBeamRayStruct *ray)
//{
//	gaussBeam_t t;
...
// * \author Mauch
// */
//geometryError ApertureArraySurface::hit(gaussBeamRayStruct &ray, gaussBeam_t t)
//{
 ...
 * \author Mauch
 */
geometryError ApertureArraySurface::reduceParams(void)
{
	if ( (this->paramsPtr!=NULL) && (this->reducedParamsPtr!=NULL) )
	{
		...
		this->reducedParamsPtr->microAptRad=this->paramsPtr->microAptRad;
		this->reducedParamsPtr->microAptPitch=this->paramsPtr->microAptPitch;
		this->reducedParamsPtr->microAptType=this->paramsPtr->microAptType;
 ...
 * \author Mauch
 */
geometryError ApertureArraySurface::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, simMode mode, double lambda )
{
	...
	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(ApertureArraySurface_ReducedParams), this->reducedParamsPtr), context) )
		return GEOM_ERR;
 ...
 * \author Mauch
 */
geometryError ApertureArraySurface::updateOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, simMode mode, double lambda )
{
	if (this->update)
	{
 ...
 * \author Mauch
 */
geometryError ApertureArraySurface::parseXml(pugi::xml_node &geometry, simMode mode, vector<Geometry*> &geomVec)
{
	Parser_XML l_parser;
	...
	if (!strcmp(str, "MICRORECTANGULAR"))
		this->paramsPtr->microAptType=AT_RECT;
	else
	{
