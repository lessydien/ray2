...
detError DetectorVolumeIntensity::detect2TextFile(FILE* hFile, RayField* rayFieldPtr)
{
	...
	{
		std::cout << "error in DetectorVolumeIntensity.detect2TextFile(): detect() returned an error" << "...\n";
		return DET_ERROR;
	}
	...
	{
		std::cout << "error in DetectorVolumeIntensity.detect2TextFile(): writeIntensityFIeld2File() returned an error" << "...\n";
		return DET_ERROR;
	}
	// clean up
...
detError DetectorVolumeIntensity::detect(Field* fieldPtr, Field **imagePtrPtr)
{
	...
	if (*imagePtrPtr != NULL)
	{
		...
		{
			std::cout << "error in DetectorVolumeIntensity.detect: given IntensityField does not match parameters of detector" << "...\n";
			return DET_ERROR;
		}
	}
	else
	{
		...
		{
			std::cout << "error in DetectorVolumeIntensity.detect: pixel number smaller than 1 in x is not allowed" << "...\n";
			return DET_ERROR;
		}
		else
		...
		{
			std::cout << "error in DetectorVolumeIntensity.detect: pixel number smaller than one in y is not allowed" << "...\n";
			return DET_ERROR;
		}
		else
		...
		{
			std::cout << "error in DetectorVolumeIntensity.detect: pixel number smaller than one in z is not allowed" << "...\n";
			return DET_ERROR;
		}
		else
		{
