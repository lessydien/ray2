...
/**\file MicroLensArraySurface.cpp
 ...
 * \author Mauch
 */
Geometry_Params* MicroLensArraySurface::getParamsPtr(void)
{
 ...
 *
 * \param[in] MicroLensArraySurface_Params *paramsInPtr
 * 
 ...
 * \author Mauch
 */
geometryError MicroLensArraySurface::setParams(Geometry_Params *paramsIn)//PlaneSurface_Params *paramsIn)
{
	MicroLensArraySurface_Params *l_ptr=dynamic_cast<MicroLensArraySurface_Params*>(paramsIn);
 ...
 * \author Mauch
 */
double MicroLensArraySurface::intersect(rayStruct *ray)
{
	double t=intersectRayMicroLensArraySurface(ray->position,ray->direction,*(this->reducedParamsPtr));
	if (t>MAX_TOLERANCE)
	{
		return t;
	}
 ...
 * \author Mauch
 */
geometryError MicroLensArraySurface::hit(rayStruct &ray, double t)
{
	Mat_hitParams hitParams;
	hitParams=calcHitParamsMicroLensArraySurface(ray.position+t*ray.direction, *(this->reducedParamsPtr));
	int i;
...
// * \author Mauch
// */
//gaussBeam_t MicroLensArraySurface::intersect(gaussBeamRayStruct *ray)
//{
//	gaussBeam_t t;
...
// * \author Mauch
// */
//geometryError MicroLensArraySurface::hit(gaussBeamRayStruct &ray, gaussBeam_t t)
//{
 ...
 * \author Mauch
 */
geometryError MicroLensArraySurface::reduceParams(void)
{
	if ( (this->paramsPtr!=NULL) && (this->reducedParamsPtr!=NULL) )
	{
		...
		this->reducedParamsPtr->microLensAptRad=this->paramsPtr->microLensAptRad;
		this->reducedParamsPtr->microLensPitch=this->paramsPtr->microLensPitch;
		this->reducedParamsPtr->microLensRadius=this->paramsPtr->microLensRadius;
		this->reducedParamsPtr->microLensAptType=this->paramsPtr->microLensAptType;
 ...
 * \author Mauch
 */
geometryError MicroLensArraySurface::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
	...
	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(MicroLensArraySurface_ReducedParams), this->reducedParamsPtr), context) )
		return GEOM_ERR;
 ...
 * \author Mauch
 */
geometryError MicroLensArraySurface::updateOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
	if (this->update)
	{
 ...
 * \author Mauch
 */
geometryError MicroLensArraySurface::parseXml(pugi::xml_node &geometry, SimParams simParams, vector<Geometry*> &geomVec)
{
	Parser_XML l_parser;
	...
	if (!strcmp(str, "MICRORECTANGULAR"))
		this->paramsPtr->microLensAptType=MICRORECTANGULAR;
	else
	{
