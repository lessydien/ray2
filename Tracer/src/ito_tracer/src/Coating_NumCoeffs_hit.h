...
#ifndef COATING_NUMCOEFFS_HIT_H
#define COATING_NUMCOEFFS_HIT_H
...
/**
  ...
  *
  */
class Coating_NumCoeffs_ReducedParams: public Coating_ReducedParams
{
public:
	double t;
	double r;
 ...
 *
 * \param[in] rayStruct &ray, Mat_hitParams hitParams, const Coating_NumCoeffs_ReducedParams& params
 * 
 * \return bool
 * \sa 
 ...
 * \author Mauch
 */
inline RT_HOSTDEVICE bool hitCoatingNumCoeff(rayStruct &ray, Mat_hitParams hitParams, const Coating_NumCoeffs_ReducedParams& params)
{
	uint32_t x1[5];
	...
	if (xi<params.r)
	{
		...
		if (params.r<max(params.t,absorb))
			ray.depth++;
	...
	else 
	{
		...
		{
			ray.running=true; // keep ray running
			if (params.t<max(params.r,absorb))
