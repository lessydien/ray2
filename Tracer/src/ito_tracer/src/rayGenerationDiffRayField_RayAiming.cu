...
rtDeclareVariable(DiffRayField_RayAimingParams, params, , );
...
//  prd.currentSeed=seed_buffer[launch_index];
  ...
  t0=clock();
	...
	//prd.direction=make_double3(0,0,1);
  ...
  for(;;) 
  {
	...
	//rtPrintf("hit pos %i: %lf %lf %lf \n", index, prd.position.x,prd.position.y,prd.position.z);
