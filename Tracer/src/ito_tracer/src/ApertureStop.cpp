 ...
 * \param[in] void
 * 
 * \return ApertureStop_Params*
 * \sa 
 * \remarks 
 * \author Mauch
 */
Geometry_Params* ApertureStop::getParamsPtr(void)
{
 ...
 * \detail setParamsPtr 
 *
 * \param[in] ApertureStop_Params *paramsInPtr
 * 
 ...
 * \author Mauch
 */
geometryError ApertureStop::setParams(Geometry_Params *paramsIn)//ApertureStop_Params *paramsIn)
{
	ApertureStop_Params *l_ptr=dynamic_cast<ApertureStop_Params*>(paramsIn);
 ...
 * \author Mauch
 */
double ApertureStop::intersect(rayStruct *ray)
{
	return intersectRayApertureStop(ray->position,ray->direction,*(this->reducedParamsPtr));
};
 ...
 * \author Mauch
 */
geometryError ApertureStop::reduceParams(void)
{
 ...
 * \author Mauch
 */
geometryError ApertureStop::hit(rayStruct &ray, double t)
{
 ...
 * \author Mauch
 */
gaussBeam_t ApertureStop::intersect(gaussBeamRayStruct *ray)
{
	gaussBeam_t t;
	t.t_baseRay=intersectRayApertureStop(ray->baseRay.position, ray->baseRay.direction, *(this->reducedParamsPtr));
    // set aperture to infinity for waist rays and divergence rays
	this->paramsPtr->apertureType=AT_INFTY;
    t.t_waistRayX=intersectRayApertureStop(ray->waistRayX.position, ray->waistRayX.direction, *(this->reducedParamsPtr));
    t.t_waistRayY=intersectRayApertureStop(ray->waistRayY.position, ray->waistRayY.direction, *(this->reducedParamsPtr));
    t.t_divRayX=intersectRayApertureStop(ray->divRayX.position, ray->divRayX.direction, *(this->reducedParamsPtr));
	t.t_divRayY=intersectRayApertureStop(ray->divRayY.position, ray->divRayY.direction, *(this->reducedParamsPtr));

	return t;//intersectRayApertureStop(ray->position,ray->direction,*(this->paramsPtr));
};
 ...
 * \author Mauch
 */
geometryError ApertureStop::hit(gaussBeamRayStruct &ray, gaussBeam_t t)
{
...
geometryError ApertureStop::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, SimParams simParams, double lambda )
{
	...
	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(ApertureStop_ReducedParams), this->reducedParamsPtr), context) )
		return GEOM_ERR;
 ...
 * \author Mauch
 */
geometryError ApertureStop::processParseResults(GeometryParseParamStruct &parseResults_Geom, int geomID)
{
...
//	this->paramsPtr->rotNormal=parseResults_Geom.rotNormal1;
	this->paramsPtr->apertureStopRadius=parseResults_Geom.obscurationHalfWidth;
	this->paramsPtr->geometryID=geomID;
	return GEOM_NO_ERR;
};

geometryError ApertureStop::parseXml(pugi::xml_node &geometry, SimParams simParams, vector<Geometry*> &geomVec)
{
	// parse base class
