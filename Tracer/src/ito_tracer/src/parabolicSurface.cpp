 ...
 * \author Mauch
 */
Geometry_Params* ParabolicSurface::getParamsPtr(void)
{
 ...
 *
 * \param[in] ParabolicSurface_Params *paramsInPtr
 * 
 ...
 * \author Mauch
 */
geometryError ParabolicSurface::setParams(Geometry_Params *paramsIn)//PlaneSurface_Params *paramsIn)
{
	ParabolicSurface_Params *l_ptr=dynamic_cast<ParabolicSurface_Params*>(paramsIn);
 ...
 * \author Mauch
 */
double ParabolicSurface::intersect(rayStruct *ray)
{
	double t=intersectRayParabol(ray->position,ray->direction,*(this->reducedParamsPtr));
	if (t>MAX_TOLERANCE)
	{
		return t;
	}
 ...
 * \author Mauch
 */
geometryError ParabolicSurface::hit(rayStruct &ray, double t)
{
	Mat_hitParams hitParams;
	hitParams=calcHitParamsParabol(ray.position+t*ray.direction, *(this->reducedParamsPtr));
	int i;
...
// * \author Mauch
// */
//gaussBeam_t ParabolicSurface::intersect(gaussBeamRayStruct *ray)
//{
//	gaussBeam_t t;
...
// * \author Mauch
// */
//geometryError ParabolicSurface::hit(gaussBeamRayStruct &ray, gaussBeam_t t)
//{
 ...
 * \author Mauch
 */
geometryError ParabolicSurface::reduceParams(void)
{
...
geometryError ParabolicSurface::createOptixInstance( RTcontext &context, RTgeometrygroup &geometrygroup, int index, simMode mode, double lambda )
{
	...
	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(ParabolicSurface_ReducedParams), this->reducedParamsPtr), context) )
		return GEOM_ERR;
 ...
 * \author Mauch
 */
geometryError ParabolicSurface::processParseResults(GeometryParseParamStruct &parseResults_Geom, int geomID)
{
...
geometryError ParabolicSurface::parseXml(pugi::xml_node &geometry, simMode mode, vector<Geometry*> &geomVec)
{
	...
	{
		std::cout << "error in ParabolicSurface.parseXml(): radius is not defined" << std::endl;
		return GEOM_ERR;
	}
	...
	{
		std::cout << "error in ParabolicSurface.parseXml(): root.x is not defined" << std::endl;
		return GEOM_ERR;
	}
	if (!l_parser.attrByNameToDouble(geometry, "root.y", l_root.y))
	{
		std::cout << "error in ParabolicSurface.parseXml(): root.y is not defined" << std::endl;
		return GEOM_ERR;
	}
	if (!l_parser.attrByNameToDouble(geometry, "root.z", l_root.z))
	{
		std::cout << "error in ParabolicSurface.parseXml(): root.z is not defined" << std::endl;
		return GEOM_ERR;
	}
