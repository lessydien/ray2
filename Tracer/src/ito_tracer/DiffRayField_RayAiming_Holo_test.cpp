...
/**\file DiffRayField_RayAiming_Holo_test.cpp
...
#include <omp.h>
#include "DiffRayField_RayAiming_Holo_test.h"
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::setLambda(double lambda)
{
 ...
 * \author Mauch
 */
unsigned long long DiffRayField_RayAiming_Holo_test::getRayListLength(void)
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::setRay(diffRayStruct ray, unsigned long long index)
{
 ...
 * \author Mauch
 */
diffRayStruct* DiffRayField_RayAiming_Holo_test::getRay(unsigned long long index)
{
	if (index <= this->rayListLength)
	{
 ...
 * \author Mauch
 */
diffRayStruct* DiffRayField_RayAiming_Holo_test::getRayList(void)
{
	return &rayList[0];	
};
 ...
 * \detail setParamsPtr 
 *
 * \param[in] DiffRayField_RayAiming_Holo_testParams *paramsPtr
 * 
 * \return void
 * \sa 
 * \remarks 
 * \author Mauch
 */
void DiffRayField_RayAiming_Holo_test::setParamsPtr(DiffRayField_RayAiming_Holo_testParams *paramsPtr)
{
 ...
 * \param[in] void
 * 
 * \return DiffRayField_RayAiming_Holo_testParams*
 * \sa 
 * \remarks 
 * \author Mauch
 */
DiffRayField_RayAiming_Holo_testParams* DiffRayField_RayAiming_Holo_test::getParamsPtr(void)
{
...
//void DiffRayField_RayAiming_Holo_test::setPathToPtx(char* path)
//{
//	memcpy(this->path_to_ptx_rayGeneration, path, sizeof(this->path_to_ptx_rayGeneration));
//};
//
//const char* DiffRayField_RayAiming_Holo_test::getPathToPtx(void)
//{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::createOptixInstance(RTcontext &context, RTbuffer &output_buffer_obj, RTbuffer &seed_buffer_obj)
{
	if (FIELD_NO_ERR != RayField::createOptixInstance(context, output_buffer_obj, seed_buffer_obj))
	{
		std::cout <<"error in DiffRayField_RayAiming_Holo_test.createOptixInstance(): RayField.creatOptiXInstance() returned an error." << std::endl;
		return FIELD_ERR;
	}
	...
	/* fill holo buffer */
	char holoAngle[128];
	sprintf( holoAngle, "%s" PATH_SEPARATOR "%s", INPUT_FILEPATH, "holo.txt" );
	std::ifstream inFile(holoAngle);

	// make sure the filestream is good
	if (!inFile)
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.createOptiXInstance(): failed to open freeform file " << std::endl;
		return FIELD_ERR;
	}
	...
	for ( unsigned int i = 0; i < (unsigned int)buffer_height; ++i )
	{
		for (unsigned int j=0; j<(unsigned int)buffer_width; ++j)
		{
			if (inFile.eof())
			{
				std::cout << "error in DiffRayField_RayAiming_Holo_test.createOptiXInstance(): end of file of freeform file before all points were read " << std::endl;
				return FIELD_ERR;
			}
	...
	{
		std::cout <<"error in DiffRayField_RayAiming_Holo_test.createOptixInstance(): create CPUSimInstance() returned an error." << std::endl;
		return FIELD_ERR;
	}
	...
	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(DiffRayField_RayAiming_Holo_testParams), this->rayParamsPtr), context) )
		return FIELD_ERR;
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::initCPUSubset()
{
	clock_t start, end;
	double msecs=0;
...
//				long long index=0; // loop counter for random rejection method
				...
				switch (this->rayParamsPtr->posDistrType)
				{
					...
					default:
						...
						std::cout << "error in DiffRayField_RayAiming_Holo_test.initCPUSubset: unknown distribution of rayposition" << std::endl;
						// report error
						break;
				}
				// interpolate phase from data table
				double l_phase;
				this->oInterpPtr->nearestNeighbour(-WIDTH_HOLO_BUFFER/2*DELTA_X_HOLO+DELTA_X_HOLO/2,-HEIGHT_HOLO_BUFFER/2*DELTA_Y_HOLO+DELTA_Y_HOLO/2,DELTA_X_HOLO,DELTA_Y_HOLO,this->holoAngle_buffer_CPU,WIDTH_HOLO_BUFFER,HEIGHT_HOLO_BUFFER,rayData.position.x,rayData.position.y,&l_phase);
				...
				rayData.opl=l_phase*this->rayParamsPtr->lambda/(2*M_PI);
...
//				rayData.position=rayData.position+epsilon*rayData.direction;
		...
		else if(this->rayParamsPtr->width*this->rayParamsPtr->height<1)
		{
			...
			std::cout << "error in DiffRayField_RayAiming_Holo_test.initCPUInstance: negative raynumber" << std::endl;
		}
		this->update=false;
	}	// end if GPUsubsetwidth*height<rayListLength
	else
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.initCPUInstance: rayList is smaller than simulation subset" << std::endl;
		return FIELD_ERR;
	}
	return FIELD_NO_ERR;
};
 ...
 * \author Mauch
 */
long2 DiffRayField_RayAiming_Holo_test::calcSubsetDim()
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::createCPUSimInstance()
{
	...
	if (!rayList)
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.createLayoutInstance(): memory for rayList could not be allocated. try to reduce ray tiling size" << std::endl;
		return FIELD_ERR;
	}
	...
	if (!inFile)
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.createCPUSimInstance(): failed to open freeform file " << std::endl;
		//return FIELD_ERR;
	}
	...
	for ( unsigned int i = 0; i < HEIGHT_HOLO_BUFFER; ++i )
	{
		for (unsigned int j=0; j<WIDTH_HOLO_BUFFER; ++j)
		{
			if (inFile.eof())
			{
				std::cout << "error in DiffRayField_RayAiming_Holo_test.createCPUSimInstance(): end of file of freeform file before all points were read " << std::endl;
				//return FIELD_ERR;
			}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::createLayoutInstance()
{
	unsigned int l_launch_width, l_launch_height, l_offsetX, l_offsetY;
	// calc the dimensions of the simulation subset
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::write2TextFile(char* filename, detParams &oDetParams)
{
	char t_filename[512];
	//sprintf(t_filename, "%s%sDiffRayField_RayAiming_Holo_test_%i.txt", filename, PATH_SEPARATOR, oDetParams.subSetNr);
	sprintf(t_filename, "%s%sDiffRayField_RayAiming_Holo_test.txt", filename, PATH_SEPARATOR);

	FILE* hFileOut;
	hFileOut = fopen( t_filename, "a" ) ;
	if (!hFileOut)
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.write2TextFile(): could not open output file: " << filename << std::endl;
		return FIELD_ERR;
	}
	// calc the dimensions of the subset
//	long2 l_GPUSubsetDim=calcSubsetDim();
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::traceScene(Group &oGroup, bool RunOnCPU, RTcontext &context, RTbuffer &output_buffer_obj, RTbuffer &seed_buffer_obj)
{
	clock_t start, end;
	double msecs=0;
	...
	start=clock();
		...
		{
			std::cout << "error in DiffRayField_RayAiming_Holo_test.traceScene: initCPUSubset returned an error" << std::endl;
			return FIELD_ERR;
		}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::traceStep(Group &oGroup, bool RunOnCPU, RTcontext &context, RTbuffer &output_buffer_obj, RTbuffer &seed_buffer_obj)
{
	clock_t start, end;
	double msecs=0;
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::copyRayList(diffRayStruct *data, long long length)
{
	if (length > this->rayListLength)
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.copyRayList(): subset dimensions exceed rayLIst dimension" << std::endl;
		return FIELD_ERR;
	}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::copyRayListSubset(diffRayStruct *data, long2 launchOffset, long2 subsetDim)
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::writeData2File(FILE *hFile, rayDataOutParams outParams)
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::convert2RayData(Field** imagePtrPtr, detParams &oDetParams)
{
	DiffRayField_RayAiming_Holo_test* l_ptr;
	// if there is no image yet, create one
	if (*imagePtrPtr == NULL)
	{
		*imagePtrPtr=new DiffRayField_RayAiming_Holo_test(this->rayListLength);
		l_ptr=dynamic_cast<DiffRayField_RayAiming_Holo_test*>(*imagePtrPtr);
	}
	else
	{
		l_ptr=dynamic_cast<DiffRayField_RayAiming_Holo_test*>(*imagePtrPtr);
	...
	DiffRayField_RayAiming_Holo_testParams *l_diffRayParamsPtr=new DiffRayField_RayAiming_Holo_testParams();
	memcpy(l_diffRayParamsPtr,this->rayParamsPtr, sizeof(DiffRayField_RayAiming_Holo_testParams));
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::convert2Intensity(Field* imagePtr, detParams &oDetParams)
{
	clock_t start, end;
...
//	long2 l_GPUSubsetDim=calcSubsetDim();
	...
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.convert2Intensity(): imagePtr is not of type IntensityField" << std::endl;
		return FIELD_ERR;
	}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::convert2ScalarField(Field* imagePtr, detParams &oDetParams)
{
	clock_t start, end;
	double msecs_Tracing=0;
	double msecs_Processing=0;
...
//	long2 l_GPUSubsetDim=calcSubsetDim();
	...
	{
		std::cout << "error in DiffRayField_RayAiming_Holo_test.convert2ScalarField(): imagePtr is not of type ScalarField" << std::endl;
		return FIELD_ERR;
	}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::convert2PhaseSpace(Field* imagePtr, detParams &oDetParams)
{
	clock_t start, end;
	double msecs_Tracing=0;
	double msecs_Processing=0;
...
// * \author Mauch
// */
//fieldError DiffRayField_RayAiming_Holo_test::convert2VecField(Field* imagePtr, detParams &oDetParams)
//{
//	return FIELD_NO_ERR;
//};
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming_Holo_test::processParseResults(FieldParseParamStruct &parseResults_Src, parseGlassResultStruct* parseResults_GlassPtr, DetectorParseParamStruct &parseResults_Det)
{
	...
	{
		std::cout <<"error in DiffRayField_RayAiming_Holo_test.processParseResults(): direction distribution GRID_RECT and GRID_RAD are not allowed if no importance area is defined" << std::endl;
		return FIELD_ERR;
	}
	//if (parseResults_Src.rayDirDistr == RAYDIR_UNIFORM)
	//{
	//	std::cout <<"error in DiffRayField_RayAiming_Holo_test.processParseResults(): direction distribution UNIFORM is not allowed for differential ray fields" << std::endl;
	//	return FIELD_ERR;
	//}
	if ( (parseResults_Src.nrRayDirections.x*parseResults_Src.nrRayDirections.y > GPU_SUBSET_WIDTH_MAX*GPU_SUBSET_WIDTH_MAX) && (parseResults_Src.rayDirDistr == RAYDIR_RAND) && ( (parseResults_Src.rayPosDistr == RAYPOS_RAND_RECT) || (parseResults_Src.rayPosDistr == RAYPOS_RAND_RAD) ) )
	{
		std::cout <<"warning in DiffRayField_RayAiming_Holo_test.processParseResults(): a number of ray directions that is bigger than the size of a GPU subset in combination with random position and direction distribution leads to a situation where some rays per point source point into the same direction when tracing on GPU." << std::endl;
	}

	//this->rayParamsPtr=new DiffRayField_RayAiming_Holo_testParams;
	...
	this->rayParamsPtr->oDetParams.apertureHalfWidth=make_double2(0.768,0.768);//parseResults_Det.apertureHalfWidth;
	...
	this->rayParamsPtr->oDetParams.root=parseResults_Det.root;
