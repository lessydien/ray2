...
/**\file DiffRayField_RayAiming.cpp
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::setLambda(double lambda)
{
 ...
 * \author Mauch
 */
unsigned long long DiffRayField_RayAiming::getRayListLength(void)
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::setRay(diffRayStruct ray, unsigned long long index)
{
 ...
 * \author Mauch
 */
diffRayStruct* DiffRayField_RayAiming::getRay(unsigned long long index)
{
	if (index <= this->rayListLength)
	{
 ...
 * \author Mauch
 */
diffRayStruct* DiffRayField_RayAiming::getRayList(void)
{
	return &rayList[0];	
};
 ...
 * \detail setParamsPtr 
 *
 * \param[in] DiffRayField_RayAimingParams *paramsPtr
 * 
 * \return void
 * \sa 
 * \remarks 
 * \author Mauch
 */
void DiffRayField_RayAiming::setParamsPtr(DiffRayField_RayAimingParams *paramsPtr)
{
 ...
 * \param[in] void
 * 
 * \return DiffRayField_RayAimingParams*
 * \sa 
 * \remarks 
 * \author Mauch
 */
DiffRayField_RayAimingParams* DiffRayField_RayAiming::getParamsPtr(void)
{
...
//void DiffRayField_RayAiming::setPathToPtx(char* path)
//{
//	memcpy(this->path_to_ptx_rayGeneration, path, sizeof(this->path_to_ptx_rayGeneration));
//};
//
//const char* DiffRayField_RayAiming::getPathToPtx(void)
//{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::createOptixInstance(RTcontext &context, RTbuffer &output_buffer_obj, RTbuffer &seed_buffer_obj)
{
	if (FIELD_NO_ERR != RayField::createOptixInstance(context, output_buffer_obj, seed_buffer_obj))
	{
		std::cout <<"error in DiffRayField_RayAiming.createOptixInstance(): RayField.creatOptiXInstance() returned an error." << std::endl;
		return FIELD_ERR;
	}
	...
	{
		std::cout <<"error in DiffRayField_RayAiming.createOptixInstance(): create CPUSimInstance() returned an error." << std::endl;
		return FIELD_ERR;
	}
	...
	if ( !RT_CHECK_ERROR_NOEXIT( rtVariableSetUserData(params, sizeof(DiffRayField_RayAimingParams), this->rayParamsPtr), context) )
		return FIELD_ERR;
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::initCPUSubset()
{
	clock_t start, end;
	double msecs=0;
...
//				long long index=0; // loop counter for random rejection method
				...
				switch (this->rayParamsPtr->posDistrType)
				{
					...
					default:
						...
						std::cout << "error in DiffRayField_RayAiming.initCPUSubset: unknown distribution of rayposition" << std::endl;
						// report error
						break;
				}
		...
		else if(this->rayParamsPtr->width*this->rayParamsPtr->height<1)
		{
			...
			std::cout << "error in DiffRayField_RayAiming.initCPUInstance: negative raynumber" << std::endl;
		}
		this->update=false;
	}	// end if GPUsubsetwidth*height<rayListLength
	else
	{
		std::cout << "error in DiffRayField_RayAiming.initCPUInstance: rayList is smaller than simulation subset" << std::endl;
		return FIELD_ERR;
	}
	return FIELD_NO_ERR;
};
 ...
 * \author Mauch
 */
long2 DiffRayField_RayAiming::calcSubsetDim()
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::createCPUSimInstance()
{
	...
	if (!rayList)
	{
		std::cout << "error in DiffRayField_RayAiming.createLayoutInstance(): memory for rayList could not be allocated. try to reduce ray tiling size" << std::endl;
		return FIELD_ERR;
	}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::createLayoutInstance()
{
	unsigned int l_launch_width, l_launch_height, l_offsetX, l_offsetY;
	// calc the dimensions of the simulation subset
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::write2TextFile(char* filename, detParams &oDetParams)
{
	char t_filename[512];
	//sprintf(t_filename, "%s%sDiffRayField_RayAiming_%i.txt", filename, PATH_SEPARATOR, oDetParams.subSetNr);
	sprintf(t_filename, "%s%sDiffRayField_RayAiming.txt", filename, PATH_SEPARATOR);

	FILE* hFileOut;
	hFileOut = fopen( t_filename, "a" ) ;
	if (!hFileOut)
	{
		std::cout << "error in DiffRayField_RayAiming.write2TextFile(): could not open output file: " << filename << std::endl;
		return FIELD_ERR;
	}
	// calc the dimensions of the subset
//	long2 l_GPUSubsetDim=calcSubsetDim();
...
//	long2 l_GPUSubsetDim=calcSubsetDim();
	...
	if (RunOnCPU)
	{
		...
		{
			std::cout << "error in DiffRayField_RayAiming.traceScene: initCPUSubset returned an error" << std::endl;
			return FIELD_ERR;
		}
...
//				oldRay.position=oldRay.position-this->rayParamsPtr->epsilon*oldRay.direction;
				...
				do
				{
					...
					this->rayList[jx]=oldRay;
					// move ray back into caustic
					this->rayList[jx].position=oldRay.position-this->rayParamsPtr->epsilon*oldRay.direction;
					...
					oldRay.mainDirX=createObliqueVec(make_double2(phi.x,phi.y+M_PI/2));
					oldRay.mainDirY=createObliqueVec(make_double2(phi.x+M_PI/2,phi.y));

					// save raydata for next iteration
					oldRay=this->rayList[jx];
					...
					double3 deltaPos=this->rayList[jx].position-hitPosOld;
					...
					if (!firstRun)
					{
						...
						// therefore calc the difference of the starting direction of this iteration (oldRay.direction) and the starting direction of the last iteration (oldDir)
						dDir_dPos=(oldRay.direction-dirOld)/deltaPos;
						// save starting direction of thsi iteration
					...
					hitPosOld=this->rayList[jx].position;
					// calc difference of current hitPos to target hitPos
					diffPos=targetHitPos-this->rayList[jx].position;
					...
					if (index>50)
					{
						std::cout << "error in DiffRayField_RayAiming.traceScene(): ray aiming loop canceled after " << index << " iterations for ray " << jx << std::endl;
						std::cout << "targetPos " << targetHitPos.x << targetHitPos.y << targetHitPos.z <<"position " << rayList[jx].position.x << rayList[jx].position.y << rayList[jx].position.z << "; direction" << rayList[jx].direction.x << rayList[jx].direction.y << rayList[jx].direction.z << std::endl;
						break;
					}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::copyRayList(diffRayStruct *data, long long length)
{
	if (length > this->rayListLength)
	{
		std::cout << "error in DiffRayField_RayAiming.copyRayList(): subset dimensions exceed rayLIst dimension" << std::endl;
		return FIELD_ERR;
	}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::copyRayListSubset(diffRayStruct *data, long2 launchOffset, long2 subsetDim)
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::writeData2File(FILE *hFile, rayDataOutParams outParams)
{
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::convert2RayData(Field** imagePtrPtr, detParams &oDetParams)
{
	DiffRayField_RayAiming* l_ptr;
	// if there is no image yet, create one
	if (*imagePtrPtr == NULL)
	{
		*imagePtrPtr=new DiffRayField_RayAiming(this->rayListLength);
		l_ptr=dynamic_cast<DiffRayField_RayAiming*>(*imagePtrPtr);
	}
	else
	{
		l_ptr=dynamic_cast<DiffRayField_RayAiming*>(*imagePtrPtr);
	...
	DiffRayField_RayAimingParams *l_diffRayParamsPtr=new DiffRayField_RayAimingParams();
	memcpy(l_diffRayParamsPtr,this->rayParamsPtr, sizeof(DiffRayField_RayAimingParams));
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::convert2Intensity(Field* imagePtr, detParams &oDetParams)
{
	clock_t start, end;
...
//	long2 l_GPUSubsetDim=calcSubsetDim();
	...
	{
		std::cout << "error in DiffRayField_RayAiming.convert2Intensity(): imagePtr is not of type IntensityField" << std::endl;
		return FIELD_ERR;
	}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::convert2ScalarField(Field* imagePtr, detParams &oDetParams)
{
	clock_t start, end;
	double msecs_Tracing=0;
	double msecs_Processing=0;
...
//	long2 l_GPUSubsetDim=calcSubsetDim();
	...
	{
		std::cout << "error in DiffRayField_RayAiming.convert2ScalarField(): imagePtr is not of type ScalarField" << std::endl;
		return FIELD_ERR;
	}
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::convert2PhaseSpace(Field* imagePtr, detParams &oDetParams)
{
	clock_t start, end;
	double msecs_Tracing=0;
	double msecs_Processing=0;
...
// * \author Mauch
// */
//fieldError DiffRayField_RayAiming::convert2VecField(Field* imagePtr, detParams &oDetParams)
//{
//	return FIELD_NO_ERR;
//};
 ...
 * \author Mauch
 */
fieldError DiffRayField_RayAiming::processParseResults(FieldParseParamStruct &parseResults_Src, parseGlassResultStruct* parseResults_GlassPtr, DetectorParseParamStruct &parseResults_Det)
{
	...
	//{
	//	std::cout <<"error in DiffRayField_RayAiming.processParseResults(): direction distribution GRID_RECT and GRID_RAD are not allowed if no importance area is defined" << std::endl;
	//	return FIELD_ERR;
	//}
	//if (parseResults_Src.rayDirDistr == RAYDIR_UNIFORM)
	//{
	//	std::cout <<"error in DiffRayField_RayAiming.processParseResults(): direction distribution UNIFORM is not allowed for differential ray fields" << std::endl;
	//	return FIELD_ERR;
	//}
