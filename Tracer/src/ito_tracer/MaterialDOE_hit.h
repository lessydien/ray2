...
//inline RT_HOSTDEVICE bool hitDOE(rayStruct &ray, Mat_hitParams hitParams, MatDOE_params params, double t_hit, int geomID, bool coat_reflected)
{
	...
	//Ort im lokalen Koordinatensystem finden
	double3 Local;
	double3 zeroposition = ray.position - root;
	rotateRayInv(&zeroposition, -tilt);
	...
	double lambda_real = 2*params.stepHeight*(params.n2-params.n1)/1000000;
	if(lambda_real == 0)
	{ 
	...
	double IndexBefore = params.n1; // Refraction indices ???????
	double IndexAfter = params.n2;
	...
	double g_projection = dot(g, hitParams.normal);
	double3 g_effective = g - g_projection * hitParams.normal;
	...
	if (abs(orders[0])>10)
	{
		return 0.0; // as for now, don't consider orders greater than 10
	}
...
//	for(j=2;j<STUETZSTELLENP-1;j++) 
	...
	if (dh==0) 
	{
		return 0; // warum auch immer das passieren sollte...
	}
...
//	u.x = ray_in.x + nn * (dPdx);	//data[ 4]  = x cosine of specular ray, on output it is the scattered ray
	u.x = ray.direction.x + (dPdx);	//data[ 4]  = x cosine of specular ray, on output it is the scattered ray
//	u.y = ray_in.y + nn * (dPdy);	//data[ 5]  = y cosine of specular ray, on output it is the scattered ray
	u.y = ray.direction.y + (dPdy);	//data[ 5]  = y cosine of specular ray, on output it is the scattered ray
	u.z = ray.direction.z;				//data[ 6]  = z cosine of specular ray, on output it is the scattered ray
	...
	ray.direction.x= u.x - (n.x*u.x + n.y*u.y + n.z*u.z)*n.x + n.x * rad;
	ray.direction.y= u.y - (n.x*u.x + n.y*u.y + n.z*u.z)*n.y + n.y * rad;
	ray.direction.z= u.z - (n.x*u.x + n.y*u.y + n.z*u.z)*n.z + n.z * rad;
	...
	else
	{
		...
		{
			ray.direction= calcSnellsLaw(ray.direction,hitParams.normal, ray.nImmersed, params.n2);
			ray.nImmersed=params.n2;
		}
		else if (ray.nImmersed==params.n2)
		{
			ray.direction= calcSnellsLaw(ray.direction,hitParams.normal, ray.nImmersed, params.n1);
