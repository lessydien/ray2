...
inline RT_HOSTDEVICE double intersectRaySphere(double3 rayPosition, double3 rayDirection, SphericalSurface_ReducedParams params)
{
  ...
  double B=2*dot(rayDirection,origin);
...
//  if (denominator!=0)
  ...
  if ( (dot(params.orientation,(intersection-params.centre))*params.curvatureRadius.x>0) || ( (abs(denominator) < 1E-10) && (abs(C) < 1E-10) ) ) // wee need to catch the case where nominator and denominator aproach zero
  {
//	// if we end up here and the ray is parallel to the orientation of the sphere we don't have any intersection. therefore returning zero
//	if (rayDirection==params.orientation)
//	{
//		return 0;
//	}
  ...
  // calc distance of intersection to optical axis of sepherical interface
  //double dist=calcDistRayPoint(params.centre, params.orientation, intersection);
  //
  //if ( dist < params.apertureRadius.x )
  //{
  //	return t;
  //}
