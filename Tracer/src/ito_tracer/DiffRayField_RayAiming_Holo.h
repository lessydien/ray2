...
/**\file DiffRayField_RayAiming_Holo.h
* \brief 
* 
*           
* \author Mauch
...
#define DIFFRAYFIELD_RAYAIMING_HOLO_PATHTOPTX "ITO-MacroSim_generated_rayGenerationDiffRayField_RayAiming_Holo.cu.ptx"
...
/**
  ...
  *
  */
class DiffRayField_RayAiming_HoloParams : public DiffRayField_RayAimingParams
{
public:
};
...
/* declare class */
/**
  *\class   DiffRayField_RayAiming_Holo
  *\ingroup Field
  *\brief   
  *
  *         
  ...
  *
  */
class DiffRayField_RayAiming_Holo : public DiffRayField_RayAiming
{
  protected:
	double3 oldPosition;
//	Pupil **pupilList;
	...
	DiffRayField_RayAiming_HoloParams *rayParamsPtr;
	...
	RTbuffer holoAbs_buffer_obj;
	double* holoAbs_buffer_CPU;

	Interpolator *oInterpPtr;

  public:
    /* standard constructor */
    DiffRayField_RayAiming_Holo()
	{
		sprintf( path_to_ptx_rayGeneration, "%s" PATH_SEPARATOR "%s", SAMPLES_PTX_DIR, DIFFRAYFIELD_RAYAIMING_HOLO_PATHTOPTX );		
		// init random seed
		...
		rayParamsPtr=new DiffRayField_RayAiming_HoloParams();
		holoAngle_buffer_CPU=NULL;

		oInterpPtr=new Interpolator();
	}
    /* Konstruktor */
    DiffRayField_RayAiming_Holo(unsigned long long length)
	{
		sprintf( path_to_ptx_rayGeneration, "%s" PATH_SEPARATOR "%s", SAMPLES_PTX_DIR, DIFFRAYFIELD_RAYAIMING_HOLO_PATHTOPTX );		
		// init random seed
		int seed = (int)time(0);            // random seed
	...
	virtual void setParamsPtr(DiffRayField_RayAiming_HoloParams *paramsPtr);
	virtual DiffRayField_RayAiming_HoloParams* getParamsPtr(void);
