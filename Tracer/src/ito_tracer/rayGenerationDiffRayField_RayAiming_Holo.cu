...
rtBuffer<double, 2>              holoAbs_buffer;
...
//  prd.currentSeed=seed_buffer[launch_index];
  ...
  t0=clock();
	...
	prd.opl=prd.opl+l_phase*prd.lambda/(2*M_PI);

	double l_flux=0;
	// modify flux according to hologram
	//nearestNeighbour_hostdevice(-WIDTH_HOLO_BUFFER/2*DELTA_X_HOLO+DELTA_X_HOLO/2,-HEIGHT_HOLO_BUFFER/2*DELTA_Y_HOLO+DELTA_Y_HOLO/2,DELTA_X_HOLO,DELTA_Y_HOLO, &holoAbs_buffer[make_uint2(0,0)], WIDTH_HOLO_BUFFER,HEIGHT_HOLO_BUFFER, prd.position.x, prd.position.y, &(l_flux));
	//prd.flux=prd.flux*l_flux;
